...
public class CartActivity extends BaseShopActivity {
    ...
    private void renderCart() {
        ...
        pigGuideController.show(Routes.CART, cartGuideContext(null), 88);
        loadCart(page);
    }
    ...
    private void renderCartContent(LinearLayout page, JSONObject cart) {
        ...
        if (pigGuideController != null) {
            pigGuideController.show(Routes.CART, cartGuideContext(cart), 88);
        }
    ...
    private JSONObject cartGuideContext(JSONObject cart) {
        ...
        if (items != null) {
            for (int i = 0; i < items.length(); i++) {
                ...
                String productId = item.optString("productId", item.optString("product_id", ""));
