...
并发语义：同一用户、同一 `product_id + sku_id` 使用原子 upsert；重复点击会累加数量，不会产生重复购物车行。
...
- 后端在事务内锁定当前用户选中的购物车项。
- 下单时校验并扣减商品/SKU 库存，库存不足则本次 checkout 失败。
- 订单、支付单、订单项、购物车清理和库存扣减在同一事务内完成。
- 并发重复 checkout 时，只有一个请求能锁定并消费已选购物车项；后续请求会因为没有选中项或库存不足而失败。
